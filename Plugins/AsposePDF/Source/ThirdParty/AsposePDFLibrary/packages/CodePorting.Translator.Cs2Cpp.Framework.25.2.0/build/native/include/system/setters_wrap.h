...
/// Contains the declaration functions that the Translator translates C# class' properties assigment expressions into.
#pragma once
...
/// Translator translates C#'s increment expressions targeting class' property that has setter and getter defined, into 
...
/// Translator translates C#'s post-increment expressions targeting class' property that has setter and getter defined, into 
/// invocation of this function. 
/// @param getter Reference to a function object representing property's getter
/// @param setter Reference to a function object representing property's setter
...
/// Translator translates C#'s decrement expressions targeting class' property that has setter and getter defined, into 
/// invocation of this function. 
/// @param getter Reference to a function object representing property's getter
/// @param setter Reference to a function object representing property's setter
...
/// Translator translates C#'s post-decrement expressions targeting a class' property that has a setter and getter defined, into 
/// invocation of this function. 
/// @param getter Reference to a function object representing property's getter
/// @param setter Reference to a function object representing property's setter
...
/// Inserts an implementation of a function into which Translator translates C#'s assignment expressions targeting a class' property that has a setter and a getter defined.
...
/// @param prop Property to set.
#if __GNUC__ > 5 && __GNUC__ < 7 // Workaround for GCC bug https://gcc.gnu.org/bugzilla/show_bug.cgi?id=66735
#define SETTER_LAMBDA(capture, prop) \
    SETTER_LVAL_LAMBDA(capture, prop)
#else
#define SETTER_LAMBDA(capture, prop) \
    [&obj = capture](const auto& v){ obj->set_##prop(v); }
#endif
...
#define GETTER_LAMBDA(capture, prop) \
    GETTER_LVAL_LAMBDA(capture, prop)
#else
#define GETTER_LAMBDA(capture, prop) \
    [&obj = capture]{ return obj->get_##prop(); }
#endif
...
/// @tparam HostSet - Host itself, or it's base type, where property's setter is defined.
...
inline typename std::enable_if<std::is_base_of<HostSet, Host>::value, T>::type
 setter_wrap(Host* const host, void (HostSet::*pSetter)(T2), T value)
{
    (host->*pSetter)(value);
    return value;
}
...
/// @returns The incremented value of the property
/// @tparam T The type of the property
/// @tparam Host - class of instance to be modified
/// @tparam HostGet - Host itself, or it's base type, where property's getter is defined
/// @tparam HostSet - Host itself, or it's base type, where property's setter is defined
template <typename T, typename Host, typename HostGet, typename HostSet>
inline typename std::enable_if<std::is_base_of<HostGet, Host>::value && std::is_base_of<HostSet, Host>::value, T>::type
setter_increment_wrap(Host* const host, T(HostGet::*pGetter)(), void (HostSet::*pSetter)(T))
{
    T tmp = (host->*pGetter)();
    (host->*pSetter)(++tmp);
    return tmp;
...
template <typename T, typename Host, typename HostGet, typename HostSet>
inline typename std::enable_if<std::is_base_of<HostGet, Host>::value && std::is_base_of<HostSet, Host>::value, T>::type
setter_increment_wrap(Host* const host, T(HostGet::*pGetter)() const, void (HostSet::*pSetter)(T))
{
    T tmp = (host->*pGetter)();
    (host->*pSetter)(++tmp);
    return tmp;
...
/// Translator translates C#'s post-increment expressions targeting instance's property that has setter and getter defined, into 
...
template <typename T, typename Host, typename HostGet, typename HostSet>
inline typename std::enable_if<std::is_base_of<HostGet, Host>::value && std::is_base_of<HostSet, Host>::value, T>::type
setter_post_increment_wrap(Host* const host, T(HostGet::*pGetter)(), void (HostSet::*pSetter)(T))
{
...
/// @tparam T The type of the property.
/// @tparam Host - class of instance to be modified
/// @tparam HostConstGet - Host itself, or it's base type, where property's getter is defined
/// @tparam HostSet - Host itself, or it's base type, where property's setter is defined
/// @param host Instance to call getters and setters for.
/// @param pGetter Function pointer pointing to the property's getter function
/// @param pSetter Function pointer pointing to the property's setter function
...
inline typename std::enable_if<std::is_base_of<HostConstGet, Host>::value && std::is_base_of<HostSet, Host>::value, T>::type
setter_post_increment_wrap(Host* const host, T (HostConstGet::*pGetter)() const, void (HostSet::*pSetter)(T))
{
    T tmp = (host->*pGetter)(), rv = tmp;
    ++tmp;
    (host->*pSetter)(tmp);
...
/// Translator translates C#'s pre-decrement expressions targeting class' property that has setter and getter defined, into 
/// invocation of this function. 
/// @param pGetter Function pointer pointing to the property's getter free function
/// @param pSetter Function pointer pointing to the property's setter free function
/// @returns The value of the property before incrementing
...
/// Translator translates C#'s pre-decrement expressions targeting instance's property that has setter and getter defined, into 
/// invocation of this function (overload for non-const getter). 
/// @tparam T The type of the property.
/// @tparam Host - class of instance to be modified
/// @tparam HostGet - Host itself, or it's base type, where property's getter is defined
...
template <typename T, typename Host, typename HostGet, typename HostSet>
inline typename std::enable_if<std::is_base_of<HostGet, Host>::value && std::is_base_of<HostSet, Host>::value, T>::type
setter_decrement_wrap(Host* const host, T(HostGet::*pGetter)(), void (HostSet::*pSetter)(T))
{
    T tmp = (host->*pGetter)();
    (host->*pSetter)(--tmp);
    return tmp;
...
template <typename T, typename Host, typename HostConstGet, typename HostSet>
inline typename std::enable_if<std::is_base_of<HostConstGet, Host>::value && std::is_base_of<HostSet, Host>::value, T>::type
setter_decrement_wrap(Host* const host, T(HostConstGet::*pGetter)() const, void (HostSet::*pSetter)(T))
{
    T tmp = (host->*pGetter)();
    (host->*pSetter)(--tmp);
    return tmp;
...
/// Translator translates C#'s post-decrement expressions targeting class' property that has setter and getter defined, into 
/// invocation of this function. 
/// @param pGetter Function pointer pointing to the property's getter free function
/// @param pSetter Function pointer pointing to the property's setter free function
/// @returns The value of the property before incrementing
...
/// Translator translates C#'s post-decrement expressions targeting instance's property that has setter and getter defined, into 
/// invocation of this function (overload for non-const getter). 
/// @tparam T The type of the property.
/// @tparam Host - class of instance to be modified
/// @tparam HostGet - Host itself, or it's base type, where property's getter is defined
...
template <typename T, typename Host, typename HostGet, typename HostSet>
inline typename std::enable_if<std::is_base_of<HostGet, Host>::value && std::is_base_of<HostSet, Host>::value, T>::type
setter_post_decrement_wrap(Host* const host, T(HostGet::*pGetter)(), void (HostSet::*pSetter)(T))
{
    T tmp = (host->*pGetter)(), rv = tmp;
    --tmp;
    (host->*pSetter)(tmp);
...
template <typename T, typename Host, typename HostConstGet, typename HostSet>
inline typename std::enable_if<std::is_base_of<HostConstGet, Host>::value && std::is_base_of<HostSet, Host>::value, T>::type
setter_post_decrement_wrap(Host* const host, T(HostConstGet::*pGetter)() const, void (HostSet::*pSetter)(T))
{
    T tmp = (host->*pGetter)(), rv = tmp;
    --tmp;
    (host->*pSetter)(tmp);
...
#define PROP_ASSIGN_WRAP(name, op) \
    ...
    template <typename PropT, typename PropValT> \
    inline PropT setter_##name##_wrap(PropT (*pGetter)(), void (*pSetter)(const PropT&), PropValT value) \
    { \
        PropT tmp = pGetter(); \
        tmp = tmp op value; \
        pSetter(tmp); \
    ...
    template <typename PropT, typename HostT, typename HostGetT, typename HostSetT, typename PropValT>                 \
    inline typename std::enable_if<std::is_base_of<HostGetT, HostT>::value && std::is_base_of<HostSetT, HostT>::value, PropT>::type \
    setter_##name##_wrap(HostT* const host, PropT (HostGetT::*pGetter)(), void (HostSetT::*pSetter)(PropT), PropValT value) \
    { \
    ...
    template <typename PropT, typename HostT, typename HostGetT, typename HostSetT, typename PropValT>                 \
    inline typename std::enable_if<std::is_base_of<HostGetT, HostT>::value && std::is_base_of<HostSetT, HostT>::value, PropT>::type \
    setter_##name##_wrap(HostT* const host, PropT (HostGetT::*pGetter)(), void (HostSetT::*pSetter)(const PropT &), PropValT value) \
    { \
        PropT tmp = (host->*pGetter)(); \
        tmp = tmp op value; \
        (host->*pSetter)(tmp); \
    ...
    template <typename PropT, typename HostT, typename HostGetT, typename HostSetT, typename PropValT> \
    inline typename std::enable_if<std::is_base_of<HostGetT, HostT>::value && std::is_base_of<HostSetT, HostT>::value, PropT>::type \
    setter_##name##_wrap(HostT* const host, PropT (HostGetT::*pGetter)() const, void (HostSetT::*pSetter)(PropT), PropValT value) \
    { \
        PropT tmp = (host->*pGetter)(); \
        tmp = tmp op value; \
        (host->*pSetter)(tmp); \
    ...
    template <typename PropT, typename HostT, typename HostGetT, typename HostSetT, typename PropValT> \
    inline typename std::enable_if<std::is_base_of<HostGetT, HostT>::value && std::is_base_of<HostSetT, HostT>::value, PropT>::type \
    setter_##name##_wrap(HostT* const host, PropT (HostGetT::*pGetter)() const, void (HostSetT::*pSetter)(const PropT &), PropValT value) \
    { \
        PropT tmp = (host->*pGetter)(); \
        tmp = tmp op value; \
        (host->*pSetter)(tmp); \
