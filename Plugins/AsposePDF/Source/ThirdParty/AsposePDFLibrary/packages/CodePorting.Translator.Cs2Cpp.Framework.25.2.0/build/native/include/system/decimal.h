...
namespace System {
    ...
    /// The number of digits in the largest decimal number that can be represented by Decimal class.
    #define ASPOSE_CPP_DECIMAL_DIGITS 28
#endif

/// Significant digits to use while converting from Double to Decimal
#define DECIMAL_FROM_DOUBLE_SIGNIFICANT_DIGITS 15

/// Significant digits to use while converting from Float to Decimal
#define DECIMAL_FROM_FLOAT_SIGNIFICANT_DIGITS 7

class IFormatProvider;

namespace Detail {
    template <typename I> class DecimalConverter;

    /// An alias for a type used to represent a decimal number.
    typedef boost::multiprecision::number<boost::multiprecision::cpp_dec_float<ASPOSE_CPP_DECIMAL_DIGITS + 1, std::int32_t, void>, (boost::multiprecision::expression_template_option)1> decimal_number_type;
    /// A data type that has a size equivalent to the size of decimal_number_type.
    struct decimal_size_equivalent {
        /// Data.
        std::array<std::uint32_t, (ASPOSE_CPP_DECIMAL_DIGITS + 8) / 8 + 3> m_data;
        /// Exponent.
        std::int32_t m_exponent;
        /// Indicates if the value is negative.
        bool m_neg;
        /// Class.
...
{
public:
    template<typename I> friend class Detail::DecimalConverter;
    ...
    /// @param mid The middle 32 bits of the value
    ...
    ASPOSECPP_SHARED_API friend std::wostream& operator<<(std::wostream& os, const Decimal& d);
    /// @endcond
    ...
    String ToString(const Decimal& value, std::nullptr_t) const { ASPOSE_UNUSED(value); return ToString(); }
    ...
    /// @returns A Decimal object representing a decimal value whose integral part is equal to that of the specified
    ...
    static bool Equals(const Decimal& d1, const Decimal& d2) { return d1 == d2; }
    ...
    static int Compare(const Decimal& d1, const Decimal& d2) { return d1.CompareTo(d2); }
    ...
    /// @returns A new instance of Decimal class representing a value equivalent to that represented by the specified string.
    ...
    /// @returns An array whose elements represent distinct bits of the binary representation of the value represented by @p d
...
private:
    /// Rounds the specified value to the nearest value with the specified number of fractional digits.
    /// Assuming "digits" and "mode" parameters are valid, so no parameters checking required.
    /// Also accepts negative "digits" values, i.e. can round 12345 to -2 digits, will result 12300)
    /// A parameter specifies the function's behavior if the specified value is equally close to two nearest numbers.
    /// @param d The value to round
    /// @param digits The number of fractional digits in the rounded value
    /// @param mode Specifies how to perform the rounding if @p value is equally close to two nearest numbers.
    /// @returns The number with the specified number of digits nearest to @p value
    static Decimal Round_unchecked(const Decimal& d, int digits, MidpointRounding mode = MidpointRounding::ToEven);
