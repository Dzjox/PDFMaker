...
public:
    ...
    /// @param dst_rect Specifies the bounding rectangle for the brush
    /// @param image_attrs The image attributes
    ASPOSECPP_SHARED_API TextureBrush(const SharedPtr<Image>& image, Rectangle dst_rect, const SharedPtr<Imaging::ImageAttributes>& image_attrs = nullptr);
    /// Constructs a new instance of TextureBrush class that uses the specified image.
    /// @param image An image used by the brush to fill the interior of a shape
    /// @param wrap_mode Specifies how the brush object is tiled
    /// @param dst_rect Specifies the bounding rectangle for the brush
...
protected:
    ...
    SharedPtr<Drawing2D::Matrix> m_transform_matrix = System::MakeObject<Drawing2D::Matrix>();
...
private:
    ...
    MEMBER_FUNCTION_MAKE_OBJECT(TextureBrush, CODEPORTING_ARGS(const TextureBrush& brush), CODEPORTING_ARGS(brush));
};
