...
namespace Details { namespace Skia { class SkPaints; } }
...
protected:
    ...
    /// @param is_closed_path The flag specifies that the path is closed.
...
private:
    ...
    /// Makes custom caps.
    Details::Skia::SkPaints MakeCustomCaps(const SkPaint& base_paint, float& start_cap_inset, float& end_cap_inset,
                            float scale_x, float scale_y) const;
    
    /// Makes main effects.
    Details::Skia::SkPaints MakeMainEffects(const SkPaint& base_paint, const SharedPtr<Drawing2D::Matrix>& matrix,
                                            float phase, float start_cap_inset, float end_cap_inset,
                                            bool is_closed_path) const;
    /// Makes compound effect.
    Details::Skia::SkPaints MakeCompoundEffect(const SkPaint& base_paint, sk_sp<SkPathEffect> dash_path_effect) const;

    /// Appends start/end line caps to the destination path.
    /// @param src_path The path to calculate line caps.
    /// @param dest_path The path where the resulting path will be added.
    /// @param start_cap The start cap.
    /// @param end_cap The end cap.
    void AppendCapsPath(const SkPath* src_path, SkPath* dest_path, SharedPtr<Drawing2D::CustomLineCap> start_cap,
        SharedPtr<Drawing2D::CustomLineCap> end_cap) const;
    ...
    /// Determines whether to apply dash caps.
    /// @param is_closed_path The flag specifies that the path is closed.
    bool IsApplyDashCaps(bool is_closed_path) const;
