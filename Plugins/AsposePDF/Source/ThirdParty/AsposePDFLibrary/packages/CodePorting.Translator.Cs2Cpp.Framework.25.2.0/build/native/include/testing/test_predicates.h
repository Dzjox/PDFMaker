...
/// Checks if type has System::Collections::Generic::IEnumerable specialization as basetype. If so, value member is set to true, otherwise
/// it is set to false.
/// @tparam T Type to check.
template<typename T, typename Enable = void>
...
/// @tparam T1 LHS element type.
/// @tparam T2 RHS element type.
/// @param lhs_expr LHS expression.
/// @param rhs_expr RHS expression.
/// @param lhs LHS value.
/// @param rhs RHS value.
...
/// Helper macros for ASPOSE_ASSERT_THROW_EXCEPTION_TYPE
...
/// or Assert.Throws(exceptionType, () => { code that can raise exception }, message);
