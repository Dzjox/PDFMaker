...
namespace Detail {
    ...
    /// @cond
    /// Declaration of template arithmetic operators for values of the FigureType enum type.
...
protected:
    ...
    /// @param matrix A matrix to use when drawing
    ...
    /// @return True if the source requires of call to ConvertCurves.
    static ASPOSECPP_SHARED_API bool NeedConvertCurves(const SkPath& source);

private:
    /// Checks whether all contours of the path are closed.
    bool IsClosedPath() const;
    /// Returns a flattened SkPath.
    std::shared_ptr<SkPath> GetFlattenPath() const;
};

}}}
/// Inserts using declarations for arithmetic operators declared in namespace System::Drawing::Drawing2D::Detail.
