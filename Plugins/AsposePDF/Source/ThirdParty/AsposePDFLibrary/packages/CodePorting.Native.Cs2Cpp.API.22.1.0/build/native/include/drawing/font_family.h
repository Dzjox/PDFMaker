...
private:
    ...
    /// Default constructor.
    FontFamily();
    MEMBER_FUNCTION_MAKE_OBJECT(FontFamily, CODEPORTING_ARGS(), CODEPORTING_ARGS());
    ...
    MEMBER_FUNCTION_MAKE_OBJECT(FontFamily, CODEPORTING_ARGS(const String &name, FontStyle init_font_style), CODEPORTING_ARGS(name, init_font_style));
    ...
    MEMBER_FUNCTION_MAKE_OBJECT_DECLARATION(FontFamily, CODEPORTING_ARGS(sk_sp<SkTypeface> sk_typeface, const SkString &name));
    ...
    /// Indicates that the passed typeface has only symbolic data
    bool IsSymbolicTypeface(const SkTypeface* typeface);
